...
const csvPath = path.join(__dirname, '..', 'src', 'data', 'stateData.csv');
const tsPath = path.join(__dirname, '..', 'src', 'data', 'stateData.ts');

const csv = fs.readFileSync(csvPath, 'utf-8').trim().replace(/\r/g, '');
...
console.log(`Generated ${tsPath} from ${csvPath} (${rows.length} states)`);
